Vue 的 `_update` 是实例的一个私有方法，它被调用的时机有 2 个，一个是首次渲染，一个是数据更新的时候；这里只分析首次渲染部分，数据更新部分会在之后分析响应式原理的时候涉及。`_update` 方法的作用是把 VNode 渲染成真实的 DOM，它的定义在 `src/core/instance/lifecycle.js` 中：

```
Vue.prototype._update = function (vnode: VNode, hydrating?: boolean) {
  const vm: Component = this
  const prevEl = vm.$el // $mount里使用query获取的
  const prevVnode = vm._vnode // 储存上次的状态，和$vnode不同
  const prevActiveInstance = activeInstance
  activeInstance = vm
  vm._vnode = vnode // 更新vnode
  // Vue.prototype.__patch__ is injected in entry points
  // based on the rendering backend used.
  if (!prevVnode) { // 假如不存在上次的状态，直接挂载到$el上
    // 初始化渲染
