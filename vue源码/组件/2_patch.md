...
### createComponent ( src/core/vdom/patch 里的)
...
if (isDef(i)) {
  // ...
  if (isDef(i = i.hook) && isDef(i = i.init)) { // 一个i变量重复使用
    i(vnode, false /* hydrating */)
    // ...
  }
  // ..
...
init (vnode: VNodeWithData, hydrating: boolean): ?boolean {
  if (
    ...
    // kept-alive components, treat as a patch里的
    const mountedNode: any = vnode // work around flow
    componentVNodeHooks.prepatch(mountedNode, mountedNode)
  } else {
    // vnode的componentInstance在这里实现
    const child = vnode.componentInstance = createComponentInstanceForVnode(
      vnode,
      activeInstance // from '../instance/lifecycle'
    )
...
function patch里的 (oldVnode, vnode, hydrating, removeOnly) {
  // ...
